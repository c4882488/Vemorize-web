       ...
       switch ($request->path()) {
            case 'doLogin':
                ...
                if(empty($email) OR empty($password)){
                    $message = '缺少必填欄位';
                    return response(["message" => $message, "status" => 403], 403);
                }
                else{
                    ...
                    if($c==1){
                        ...
                        $message = '登入成功';
                        return response(["message"=>$message, 'token'=> $token, "status" => 200], 200);
                    }
                    else{
                        $message = '帳號或密碼錯誤';
                        return response(["message" => $message, "status" => 403], 403);
            ...
            case 'register':
                ...
                if(empty($name) OR empty($email) OR empty($password) OR empty($birthday)){
                    $message = '缺少必填欄位';
                    return response(["message" => $message, "status" => 400], 400);
                }
                else{
                    $this->usermodel = new UserModel();
                    ...
                    if($c==1){
                        $message = '信箱已存在';
                        return response(["message" => $message, "status" => 409], 409);
                    }
                    else{
                        ...
                        if($response){
                            $message = '註冊成功';
                            return response(["message" => $message, "status" => 200], 200);
                        }
                        else{
                            $message = 'SQL錯誤';
                            return response(["message" => $message, "status" => 405], 405);
                        }
            ...
            case 'checkEmail':
                ...
                if(empty($email)){
                    ...
                    if(count($response)!=0){
                        $message = '信箱已存在';
                        return response(["message" => $message, "status" => 406], 406);
                    }
                    else{
                        $message = '信箱可註冊';
                        return response(["message" => $message, "status" => 200], 200);
            ...
            default:
                ...
                if(empty($jwtToken)){
                    ...
                    if($this->checkToken($request)){
                        ...
                        $message = '無效Token';
                        return response(["message" => $message, "status" => 401], 401);
                    }
                    break;
                }
